clc; clear; close all;
f=20;I=imread('MoonLanding.bmp');[X, Y]=size(I);
...
figure(2); imshow(JFT);title('FFT of Original Image','FontSize',f);
figure(3);imagesc(H);colormap(gray);
title('Filter in freq domain','FontSize',f);
figure(4);imshow(JFR);title('Filtered Image','FontSize',f);
figure(5); imshow(JFF);title('FFT of filtered Image','FontSize',f);

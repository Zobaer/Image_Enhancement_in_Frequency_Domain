...
u3=136; v3=127; u4=160; v4=169;D01=750;D02=350;
n = 2; %Order of the Butterworth filter
H = ones(size(I));
for i=1:X
    for j=1:Y
        D1=(i-u0)^2+(j-v0)^2; D2=(i-u1)^2+(j-v1)^2;
        D3=(i-u3)^2+(j-v3)^2; D4=(i-u4)^2+(j-v4)^2;
        H1(i,j) =1/(1+(D01^2/(D1*D2))^(2*n));
        H2(i,j) =1/(1+(D02^2/(D3*D4))^(2*n));
        H(i,j) = H1(i,j)*H2(i,j);
    end
end
JFS=FS.*H;
